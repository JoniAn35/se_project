...
public class OwnerServiceImpl implements OwnerService {
   ...
   * Creates a new owner.
   *
   * @param newOwner the details of the owner to create
   * @return the created {@link OwnerDto}
  ...
  public OwnerDto create(OwnerCreateDto newOwner) throws ValidationException {
    ...
    if (newOwner.firstName() == null || newOwner.firstName().isBlank()) {
      validationErrors.add("Owner first name is required and cannot be blank");
    }

    // Validate last name (required)
    if (newOwner.lastName() == null || newOwner.lastName().isBlank()) {
      validationErrors.add("Owner last name is required and cannot be blank");
    }
